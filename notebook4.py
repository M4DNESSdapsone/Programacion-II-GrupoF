...
ventanaPrincipal.geometry("700x570")
...
def guardar_en_archivo():
    with open ("paciente.txt","w",encoding="utf-8")as archivo: #(la w se usa para escritura\ encoding es para que se acepten tildes o "ñ")
        for paciente in paciente_data:
            archivo.write(
                
                f"{paciente['Nombre']}|{paciente['Fecha de nacimiento']}|{paciente['Edad']}|{paciente['Genero']}|{paciente['Grupo Sanguineo']}|{paciente['Tipo de seguro']}|{paciente['Centro médico']}\n")
...
def eliminar_Paciente():
    ...
    if seleccionado:
        ...
        if messagebox.askyesno("Eliminar Paciente", f"¿Esta seguro de eliminar el paciente'{treeview.item(id_item, 'values'[0])}'?"):
            ...
            guardar_en_archivo()
            cargar_treeview()
            messagebox.showinfo("Eliminar Paciente", "Paciente eliminado con exito! ;D")
    else:
        messagebox.showwarning("Eliminar Paciente", "No se ha seleccionado ningun paciente! :c")
        return
...
def guardar_en_archivoDoc():
    with open ("doctor.txt","w",encoding="utf-8")as archivo:
        for doctor in doctores_data:
            archivo.write(f"{doctor['Nombre']}|{doctor['Especialidad']}|{doctor['Telefono']}|")
...
btn_registrarD.configure(bg="Lightblue")

#boton eliminar
btn_eliminarD=tk.Button(btn_frameD,text="Eliminar", command= eliminar_Doctor)
btn_eliminarD.grid(row=5,column=1,padx=5)
btn_eliminarD.configure(bg="pink")
