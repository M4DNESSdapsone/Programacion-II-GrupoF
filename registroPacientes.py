...
def enmascarar_fecha(texto):
    ...
    if fecha_N.get() != formato_final:
        fecha_N.delete(0, tk.END)
        fecha_N.insert(0, formato_final)

    if len(fecha_N.get()) == 10:
        fecha_actual = datetime.now().date()
        fecha_nacimiento = datetime.strptime(fecha_N.get(), "%d-%m-%Y").date()
...
nombre_P = tk.Entry(framePacientes)
nombre_P.grid(row=0, column=1, padx=5, pady=5, sticky="w")
...
fecha_N = ttk.Entry(framePacientes, validate="key", validatecommand=(validacion_fecha, '%P'))
fecha_N.grid(row=1, column=1, padx=5, pady=5, sticky="w")
...
comboTipoS = ttk.Combobox(framePacientes, values=["Publico", "Privado", "Ninguno"], textvariable=tipoSeguro)
...
estatura_p = tk.Entry(framePacientes)
estatura_p.grid(row=8, column=1, padx=5, pady=5, sticky="w")
...
# Botón Registrar
btn_Registrar = tk.Button(btnFrame, text="Registrar", command=registrarPaciente)
btn_Registrar.grid(row=0, column=0, padx=5)
btn_Registrar.configure(bg="lightblue")

# Botón Eliminar
btn_Eliminar = tk.Button(btnFrame, text="Eliminar", command=eliminar_paciente)
btn_Eliminar.grid(row=0, column=1, padx=5)
btn_Eliminar.configure(bg="pink")
